...
- [ ] 每轮审查最多 3 次迭代，超限上报用户
- [ ] 用户验收模糊信号必须追问，不能假设批准
- [ ] 最终用户确认通过后才调用 `sw-verification-before-completion`
...
**请求用户最终批准**：
- "所有任务已完成，整体审查通过。是否批准进入 `sw-verification-before-completion` 标记完成？"
...
- **如果用户批准**：调用 `sw-verification-before-completion` Skill
...
[最终用户确认]
你: "所有 5 个任务已完成。整体审查通过。是否批准进入 sw-verification-before-completion 标记完成？"
用户: "批准"
